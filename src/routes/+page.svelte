<script lang="ts">
    ...
    class Line {
        public name: string;
        public color: string;
        public hasBlackText: boolean;
        public reliability: Map<number, number>;

        public constructor(name: string, color: string, hasBlackText: boolean, performance: Array<number>) {
            if (performance.length != 10) {
                throw `${name} has array of ${performance.length} years, expected 10`
            }
            this.name = name;
            this.color = color;
            this.hasBlackText = hasBlackText;
